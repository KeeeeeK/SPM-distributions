...
def constant_phase_curve_2signs(Z: complex = 1 + 1j,
    ...
    for k in k_range:
        ...
        # else:
        #     axes.annotate(f'$k={k}$', xy=(x_k, y_k), xytext= (x_k - 2, y_k + 0.5))
        for gamma_sign in (-1, 1):
            points = constant_phase_curve(z, analytic_func * gamma_sign, (x_k, y_k), steps_params=steps_params)
            plt.plot(*zip(*points), color=f'C{0 if gamma_sign == 1 else 1}', linewidth=0.85)
...
def _arrows(axes, x_min, x_max, y_min, y_max, dy: float) -> None:
    ...
    axes.annotate('', xy=(0.5, 1.035), xycoords='axes fraction', xytext=(0.5, 0.99), arrowprops=arrowprops)
...
def default_figsize(x_min, x_max, y_min, y_max):
    rescale = (y_max - y_min) / (x_max - x_min)
    width = 3.15
    return width, rescale * width
